#include "myLib.h"
#include <stdio.h>
#include <stdlib.h>
#include "text.h"
unsigned short *videoBuffer = (unsigned short*)0X6000000;

void waitForVblank() {
	while (SCANLINECOUNTER < 160) {}
	while (SCANLINECOUNTER > 160) {}
}

void fillScreen(unsigned short color) {
	for (int i=0; i < 240; i++) {
		for (int j=0; j < 160; j++) {
			videoBuffer[i+240*j] = color;
		}
	}
}

void drawChar(int row, int col, char ch, u16 color)
{
	for(int r=0; r<8; r++)
	{
		for(int c=0; c<6; c++)
		{
			if(fontdata_6x8[OFFSET(r, c, 6)+ch*48])
			{
				setPixel(row+r, col+c, color);
			}
		}
	}
}

void drawString(int row, int col, char *str, u16 color)
{
	while(*str)
	{
		drawChar(row, col, *str++, color);
		col += 6;
		
	}
}



//not really used
void drawBackground(const unsigned short *image) {
	DMA[3].src = image;
	DMA[3].dst = videoBuffer;
	DMA[3].cnt = (240*160) | DMA_ON | DMA_SOURCE_INCREMENT | DMA_DESTINATION_INCREMENT;
}

//draws bmp images into GBA window
//this is my drawImage3 but just without the 3 in the name
void drawImage(int row, int col, int width, int height, const u16* image) {
	for (int r=0; r<height; r++) {
		//REG_DMA3SAD = (u32)&image[r*width];
		//REG_DMA3DAD = (u32)(&videoBuffer[OFFSET(row+r, col, 240)]);
		//REG_DMA3CNT = width | DMA_ON;
		DMA[3].src = (u32*)&image[r*width];
		DMA[3].dst = (u32*)(&videoBuffer[OFFSET(row+r, col, 240)]);
		DMA[3].cnt = (u32)(width | DMA_ON | DMA_SOURCE_INCREMENT | DMA_DESTINATION_INCREMENT);
	}
}

void setPixel(int row, int col, unsigned short color)
{
	videoBuffer[OFFSET(row,col, 240)] = color;
}

//for erasing trails of moving harambe
void drawRect(int row, int col, int height, int width, volatile unsigned short color)
{
	for(int r=0; r<height; r++)
	{
		DMA[3].src = (u32*)&color;
		DMA[3].dst = (u32*)(&videoBuffer[OFFSET(row+r, col, 240)]);
		DMA[3].cnt = width | DMA_ON | DMA_SOURCE_FIXED;
	}
}